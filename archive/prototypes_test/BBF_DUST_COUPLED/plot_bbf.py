#!/usr/bin/env python3
"""Analyze one coupled MBDyn/DUST trim and short-period/1SWB run."""
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot one coupled BBF MBDyn/DUST NetCDF result")
    parser.add_argument("result", nargs="?", default="output/V_030.00_mps/case.nc")
...
def result_constant(result: Path, name: str, fallback: float) -> float:
    ...
    if log_path.is_file():
        ...
        if np.isfinite(value):
            return value
    main = (Path(__file__).parent / "main_bbf_dust.mbd").read_text(encoding="utf-8")
