...
def expanded_input_text(path: Path, seen: set[Path] | None = None) -> str:
    """Read an MBDyn input together with its recursively included text files."""
    ...
    chunks = [text]
    for include_name in re.findall(r'(?m)^\s*include:\s*"([^"]+)"\s*;', text):
        include_path = Path(include_name)
        if not include_path.is_absolute():
            include_path = resolved.parent / include_path
        if include_path.is_file():
            chunks.append(expanded_input_text(include_path, visited))
    return "\n".join(chunks)
...
def analyze_case(nc_path: Path, bff_target: float | None = None) -> Summary:
    ...
    text = expanded_input_text(input_path)
