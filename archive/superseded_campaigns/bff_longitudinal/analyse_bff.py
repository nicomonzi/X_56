...
def filter_second_order(
    signal: np.ndarray, a1: float, a2: float, b0: float, b1: float, b2: float
) -> np.ndarray:
    """Replica un ``discrete filter`` MBDyn del secondo ordine."""
    output = np.zeros_like(signal, dtype=float)
    for i, value in enumerate(signal):
        y1 = output[i - 1] if i >= 1 else 0.0
        y2 = output[i - 2] if i >= 2 else 0.0
        u1 = signal[i - 1] if i >= 1 else 0.0
        u2 = signal[i - 2] if i >= 2 else 0.0
        output[i] = a1 * y1 + a2 * y2 + b0 * value + b1 * u1 + b2 * u2
    return output


def longitudinal_bandstop(signal: np.ndarray, text: str) -> np.ndarray:
    """Applica le quattro sezioni usate dal SAS longitudinale MBDyn."""
    value = np.asarray(signal, dtype=float)
    for section in range(1, 5):
        prefix = f"BFF_BS{section}_"
        value = filter_second_order(
            value,
            constant(text, prefix + "A1"), constant(text, prefix + "A2"),
            constant(text, prefix + "B0"), constant(text, prefix + "B1"),
            constant(text, prefix + "B2"),
        )
    return value
...
def analyze_case(nc_path: Path, bff_target: float | None = None) -> Summary:
    ...
    # I casi salvati mantengono gli include separati: accodare setconst rende
    # disponibili al post-processore gli stessi coefficienti usati da MBDyn.
    ...
    with Dataset(nc_path) as data:
        ...
        pitch_bff_free = longitudinal_bandstop(pid[9103], text)
        q_bff_free = longitudinal_bandstop(pid[9104], text)
        long_raw = pitch_bff_free + q_bff_free + safety_raw
        ...
        long_baseline = filter_first_order(q_bff_free, a1, b0, b1)
        long_outer = filter_first_order(pitch_bff_free, a1, b0, b1)
    ...
    sample_frequency = 1.0 / constant(text, "TIME_STEP")
    response = np.ones(8192, dtype=complex)
    w = None
    for section in range(1, 5):
        prefix = f"BFF_BS{section}_"
        b = [constant(text, prefix + "B0"), constant(text, prefix + "B1"), constant(text, prefix + "B2")]
        a = [1.0, -constant(text, prefix + "A1"), -constant(text, prefix + "A2")]
        w, section_response = freqz(b, a, worN=8192, fs=sample_frequency)
...
def make_clean_plots(
    ...
    axes[0].plot(time, altitude, color="tab:blue", lw=1.6)
    axes[0].axhline(0.0, color="0.25", lw=0.7)
    axes[0].set_ylabel("quota relativa [m]")

    axes[1].plot(time, euler[:, 1], color="tab:orange", lw=1.5)
    axes[1].axhline(0.0, color="0.25", lw=0.7)
    axes[1].set_ylabel("pitch [deg]")

    for index, label, color in ((0, "p", "tab:blue"), (1, "q", "tab:orange"), (2, "r", "tab:green")):
        axes[2].plot(time, omega[:, index], label=label, color=color, lw=1.25)
    axes[2].set_ylabel("ratei [deg/s]")
    axes[2].legend(ncol=3, loc="upper right")

    surface_series = (
        ("BFL", "body flap L", "tab:blue", "-"),
        ("BFR", "body flap R", "tab:orange", "--"),
        ("WF1L", "WF1/WF2 L", "tab:green", "-"),
        ("WF1R", "WF1/WF2 R", "tab:red", "--"),
        ("WF4L", "WF4 L=R (burst)", "tab:purple", "-"),
    )
    for name, label, color, style in surface_series:
        axes[3].plot(time, surfaces[name] * RAD_TO_DEG, label=label, color=color, ls=style, lw=1.15)
    axes[3].set_ylabel("superfici [deg]")
    ...
    for axis in axes:
        ...
        axis.margins(x=0)
    fig.suptitle(f"V={velocity:.2f} m/s   q={q_nominal:.1f} Pa   (fascia grigia: burst)")
    ...
    axes[0].plot(ti, mode7, color="tab:blue", lw=1.45)
    axes[0].axhline(0.0, color="0.25", lw=0.7)
    axes[0].text(
    ...
    axes[1].plot(ti, pitch, color="tab:orange", lw=1.25)
    axes[1].axhline(0.0, color="0.25", lw=0.7)
    axes[1].set_ylabel("Δpitch [deg]")
    axes[1].set_xlabel("tempo post-burst [s]")

    for axis in axes:
        axis.grid(alpha=0.22)
        axis.margins(x=0)
    fig.suptitle(f"Mode 7 e pitch — V={velocity:.2f} m/s, q={q_nominal:.1f} Pa")
...
def write_study(summaries: list[Summary]) -> None:
    ...
    ax[0].plot(q[bff_valid], f[bff_valid], "o-", label="ramo BFF valido")
    ax[0].plot(q[~bff_valid], f[~bff_valid], "x", color="0.55", label="caso escluso")
    ax[0].axhline(NASTRAN_FF, color="k", ls="--", label="NASTRAN"); ax[0].set_ylabel("f BFF [Hz]"); ax[0].legend()
    ax[1].plot(q[bff_valid], sigma[bff_valid], "o-", label="BFF")
    ax[1].plot(q[~bff_valid], sigma[~bff_valid], "x", color="0.55")
    ax[1].axhline(0, color="k", lw=.8); ax[1].set_ylabel("sigma BFF [1/s]")
    ax[2].plot(q[rigid_valid], rigid_sigma[rigid_valid], "o-", label="longitudinale rigido")
    ax[2].plot(q[~rigid_valid], rigid_sigma[~rigid_valid], "x", color="0.55")
    ax[2].axhline(0, color="k", lw=.8); ax[2].set_ylabel("sigma rigido [1/s]")
    ax[2].set_xlabel("q nominale [Pa]")
    ...
    report = [
        ...
        "I modi FEM 1–6 sono rigidi e sono esclusi perché il moto rigido è rappresentato dal floating frame MBDyn; il joint esterno rimuove poi soltanto X e Y. Sono attivi i primi 25 modi elastici FEM 7–31 (3.217–36.970 Hz).",
    ...
    report += [
        ...
        "- SAS longitudinale: quattro notch in cascata a 1.3/1.6/1.9/2.1 Hz, Q=2, fs=200 Hz; attenuazione almeno 20 dB nella banda BFF 1.25–2.2 Hz;",
...
- Modi FEM attivi: 7–31, i primi 25 elastici. Modi 1–6: rigidi esclusi. Campo di frequenza secco: 3.2171–36.9701 Hz.
